...
function Skeleton() {
  return (
    <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:16, overflow:"hidden" }}>
      <div className="skeleton" style={{ width:"100%", height:170, background:"var(--border)", opacity:.5 }} />
...
function HomeInner() {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db,"views")).then(snap => {
      ...
      snap.docs.forEach(d => { m[d.id] = d.data().count || 0; });
      setViews(m);
    });
  }, []);
                ...
                <div key={p.id} className="card" style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:16, overflow:"hidden", display:"flex", flexDirection:"column" }}>
                  ...
                  <div style={{ padding:"0 20px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                      ...
                      {views[p.id] ? <span style={{ fontSize:11, color:"var(--sub)" }}>· {views[p.id]} views</span> : null}
                    </div>
